...
class CheckPlaceAPIControllerTest {
    ...
    void there_is_no_place_in_the_area_and_return_not_found() throws Exception {
        Optional<Auth> oauth = Optional.of(auth);
        Mockito.when(authRepository.findByUsername(any())).thenReturn(oauth);
    ...
    void user_is_in_correct_place() throws Exception {
        ...
        assertEquals("EU.000.000.000.00000", list.get(0).getId());
        assertEquals("EU.POL.000.000.00000", list.get(1).getId());
        assertEquals("EU.POL.DLS.000.00000", list.get(2).getId());
        assertEquals("EU.POL.DLS.WRO.00000", list.get(3).getId());
        assertEquals("EU.POL.DLS.WRO.00003", list.get(4).getId());
    }
    ...
    @Test
    void user_already_been_in_other_place_in_the_same_area() throws Exception {
        Map<String, Integer> achievedPlaces = new HashMap<>();
    ...
    void there_are_two_places_in_the_user_range() throws Exception {
        ...
        assertEquals("EU.POL.DLS.000.00000", list.get(2).getId());
        assertEquals("EU.POL.DLS.WRO.00000", list.get(3).getId());
        assertEquals("EU.POL.DLS.WRO.00001", list.get(4).getId());
        assertEquals("EU.POL.DLS.WRO.00002", list.get(5).getId());
