...
public class RegistrationController {
    ...
    public String checkResetCode(@PathVariable("code") String code, Principal principal, RedirectAttributes ra){
        ...
        try {
            ...
            ra.addAttribute("msg_info", "Konto zostało aktywowane");
        }catch (HttpClientErrorException e){
            if(e.getStatusCode().equals(HttpStatus.NOT_FOUND)){
                ra.addAttribute("msg_info", "Błędny kod aktywacyny. Może został juz wykorzystany.");
            }else if(e.getStatusCode().is5xxServerError()){
                ra.addAttribute("msg_err", "Błąde serwera");
            }else {
                ra.addAttribute("msg_err", "Coś poszło nie tak");
            }
        }
